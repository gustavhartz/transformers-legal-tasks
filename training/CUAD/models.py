...
class QAModelFC(nn.Module):
    def __init__(self,
                 hparams,
                 transformerQA):

        super(QAModelFC, self).__init__()
        self.hparams = hparams
        self.model = getattr(transformerQA, hparams.get('model_type'))

        # 768 and 2 from the normal bert config
        self.linearOut = transformerQA.qa_outputs
        self.linearFinal = nn.Linear(2 *
                                     hparams.get('max_seq_length'), 2 *
                                     hparams.get('max_seq_length'))
...
class QAModelFCOnly(nn.Module):
    def __init__(self,
                 hparams,
                 transformerQA):

        super(QAModelFCOnly, self).__init__()
        self.hparams = hparams
        self.model = getattr(transformerQA, hparams.get('model_type'))
        self.fc_out = nn.Linear(hparams.get('hidden_size') *
                                hparams.get('max_seq_length'), 2 *
                                hparams.get('max_seq_length'))
