...
import time
from utils_valid import decode
...
def compute_predictions_logits_multi(
    ...
    pred_func = cuad_convert_example_output_to_prediction
    if pred_logic == 'v2':
        pred_func = cuad_convert_example_output_to_prediction_v2
...
def cuad_convert_example_output_to_prediction_v2(
    example,
    n_best_size,
    max_answer_length,
    do_lower_case,
    ...
    _PrelimPrediction = collections.namedtuple(  # pylint: disable=invalid-name
        "PrelimPrediction", ["feature_index", "start_index",
                             "end_index", "start_logit", "end_logit", "score"]
    )
    ...
    for (feature_index, feature) in enumerate(features):
        result = unique_id_to_result[feature.unique_id]
        start_, end_, p_mask = result.start_logits, result.end_logits, feature.p_mask
        ...
        if version_2_with_negative:
            feature_null_score = start_[0] * end_[0].item()
            if feature_null_score < score_null:
                score_null = feature_null_score
                min_null_feature_index = feature_index
                null_start_logit = result.start_logits[0].item() if torch.is_tensor(
                    null_start_logit) else result.start_logits[0]
                null_end_logit = result.end_logits[0] if torch.is_tensor(
                    null_end_logit) else result.end_logits[0]

        # Mask CLS
        start_[0] = end_[0] = 0.0

        # Extract predictions
        starts, ends, scores = decode(start_ if torch.is_tensor(start_) else torch.tensor(start_), end_ if torch.is_tensor(end_) else torch.tensor(end_), max(
            n_best_size*2, 20), max_answer_length)
        for s, e, score in zip(starts, ends, scores.squeeze()):
            prelim_predictions.append(
                _PrelimPrediction(
                    feature_index=feature_index,
                    start_index=s.item() if torch.is_tensor(s) else s,
                    end_index=e.item() if torch.is_tensor(e) else e,
                    start_logit=result.start_logits[s].item() if torch.is_tensor(
                        result.start_logits[s]) else result.start_logits[s],
                    end_logit=result.end_logits[e].item() if torch.is_tensor(
                        result.end_logits[e]) else result.end_logits[e],
                    score=score.item(),
    ...
    for pred in prelim_predictions:
        ...
        nbest.append(_NbestPrediction(
            text=final_text, start_logit=pred.start_logit.item() if torch.is_tensor(pred.start_logit) else pred.start_logit, end_logit=pred.end_logit.item() if torch.is_tensor(pred.end_logit) else pred.end_logit, score=pred.score))

    # if we didn't include the empty option in the n-best, include it
    if version_2_with_negative:
        if "" not in seen_predictions:
            nbest.append(_NbestPrediction(
                text="", start_logit=null_start_logit.item() if torch.is_tensor(null_start_logit) else null_start_logit, end_logit=null_end_logit.item() if torch.is_tensor(null_end_logit) else null_end_logit, score=score_null.item() if torch.is_tensor(score_null) else score_null))
            start_indexes.append(-1)
            end_indexes.append(-1)
        ...
        if len(nbest) == 1:
            nbest.insert(0, _NbestPrediction(
                text="empty", start_logit=0.0, end_logit=0.0, score=0))
            start_indexes.append(-1)
            end_indexes.append(-1)
    ...
    if not version_2_with_negative:
        ...
        # predict "" iff the null score - the score of best non-null > threshold
        score_diff = score_null - best_non_null_entry.score
...
def get_balanced_dataset_v2(dataset, tokenizer, keep_frac=1, return_positives_dict=False):
    ...
    for idx, ele in tqdm(enumerate(dataset), desc='Finding negative examples', total=len(dataset)):
        ...
        # Get question category - Only works for original CUAD
